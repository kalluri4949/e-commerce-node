...
- [] use findOneAndUpdate() - will not trigger hook
- [] create token user, attachCookiesToResponse and send back the tokenUser

#### updateUser with user.save() - will trigger hook so add condition like isModified('password)
...
- import status codes, Product Model, Custom Error
